...
func (s UInt8Set) IsProperSubsetOf(t UInt8Set) bool {
	for e := range s {
		if _, ok := t[e]; !ok {
			return false
		}
...
func (s UInt8Set) IsSupersetOf(t UInt8Set) bool {
	for e := range t {
		if _, ok := s[e]; !ok {
			return false
		}
...
func (s UInt8Set) IsProperSupersetOf(t UInt8Set) bool {
	for e := range t {
		if _, ok := s[e]; !ok {
			return false
		}
