...
func (s Int64Set) IsProperSubsetOf(t Int64Set) bool {
	for e := range s {
		if _, ok := t[e]; !ok {
			return false
		}
...
func (s Int64Set) IsSupersetOf(t Int64Set) bool {
	for e := range t {
		if _, ok := s[e]; !ok {
			return false
		}
...
func (s Int64Set) IsProperSupersetOf(t Int64Set) bool {
	for e := range t {
		if _, ok := s[e]; !ok {
			return false
		}
