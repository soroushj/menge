...
func (s Float64Set) IsProperSubsetOf(t Float64Set) bool {
	for e := range s {
		if _, ok := t[e]; !ok {
			return false
		}
...
func (s Float64Set) IsSupersetOf(t Float64Set) bool {
	for e := range t {
		if _, ok := s[e]; !ok {
			return false
		}
...
func (s Float64Set) IsProperSupersetOf(t Float64Set) bool {
	for e := range t {
		if _, ok := s[e]; !ok {
			return false
		}
