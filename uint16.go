...
func (s UInt16Set) IsProperSubsetOf(t UInt16Set) bool {
	for e := range s {
		if _, ok := t[e]; !ok {
			return false
		}
...
func (s UInt16Set) IsSupersetOf(t UInt16Set) bool {
	for e := range t {
		if _, ok := s[e]; !ok {
			return false
		}
...
func (s UInt16Set) IsProperSupersetOf(t UInt16Set) bool {
	for e := range t {
		if _, ok := s[e]; !ok {
			return false
		}
